...
$myToolbar = array($moduleid.'settemplate',$moduleid.'refresh');
...
<script>
	...
	function <?php echo $templatedetailid.$submod; ?>_%formval%() {
		...
		//var myToolbar = ['promotionnew','promotionedit','promotiondelete','promotionsave','promotioncancel','promotionrefresh','promotiondetails'];
...
///////////////////////////////////
		...
		myForm.setItemFocus("promos_title");

		if(myForm.getItemValue('promos_startdate')==null) {

			var date = new Date();

			myForm.setItemValue('promos_startdate',date);

			date.setDate(date.getDate() + 1);

			myForm.setItemValue('promos_enddate',date);

			cal = myForm.getCalendar('promos_enddate');

			cal.setSensitiveRange(date, null);

		} else {

			var cal = myForm.getCalendar('promos_startdate');

			var date = new Date(cal.getDate());
...
///////////////////////////////////
		...
		var myCalendar = myForm.getCalendar('promos_startdate');
		...
		myCalendar.attachEvent("onBeforeChange", function(date){
			...
			var cal = myForm.getCalendar('promos_enddate');
		...
		//myCalendar.disableDays('year',[2015,2016,2017]);

		//myCalendar.setSensitiveRange("06-10-2016 00:00", "06-15-2016 00:00");
		...
		myForm.attachEvent("onValidateError", function(id,value){
			...
			if(id=='promos_title') {
				msg = 'Please enter Title. This field is required.';
			} else
			if(id=='promos_desc') {
				msg = 'Please enter Description. This field is required.';
			} else
			if(id=='promos_sms') {
				msg = 'Please enter SMS. This field is required.';
			}
		...
		myTab.toolbar.getToolbarData('<?php echo $moduleid; ?>settemplate').onClick = function(id,formval) {
		...
		/*myTab.toolbar.getToolbarData('promotiondetails').onClick = function(id,formval) {
			//showMessage("toolbar: "+id,5000);
			//doSelect_%formval%("promos");

			//var myTab = srt.getTabUsingFormVal('%formval%');

			if(!myTab.layout.cells('c').isCollapsed()) {
