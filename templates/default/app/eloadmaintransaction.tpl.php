...
<?php /* ?>
$rows[] = array('id'=>$v['eloadtransaction_id'],'data'=>array(0,$v['eloadtransaction_id'],$v['eloadtransaction_productcode'],$v['eloadtransaction_productdesc'],$v['eloadtransaction_mobileno'],$v['eloadtransaction_message'],$v['eloadtransaction_confirmation'],$v['eloadtransaction_reference'],$v['eloadtransaction_cost'],$v['eloadtransaction_actualcost'],$v['eloadtransaction_balance'],pgDate($v['eloadtransaction_createstamp']),pgDate($v['eloadtransaction_updatestamp'])));
<?php */ ?>
			var myGrid = myGrid_%formval% = new dhtmlXGridObject('<?php echo $templatemainid.$submod; ?>grid');

			myGrid.setImagePath("/codebase/imgs/")

			myGrid.setHeader("#master_checkbox,ID, Product, Recipient, SMS, Confirmation, Reference, Cost, ActualCost, Balance, Completed, Created, Updated");

			myGrid.setInitWidths("50,70,100,150,200,200,150,100,100,100,100,150,150");

			myGrid.setColAlign("center,center,left,left,left,left,left,left,left,left,left,left,left");
			...
			try {
				...
				if(ddata.rows[0].id) {
					...
					myGrid.attachHeader("&nbsp;,&nbsp;,#combo_filter,#combo_filter,#text_filter,#text_filter,#text_filter,#text_filter,&nbsp;,&nbsp;,&nbsp;,&nbsp;,&nbsp;,&nbsp;");
