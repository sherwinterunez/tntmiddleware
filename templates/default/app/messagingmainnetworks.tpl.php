<div id="messagingmain">
	<div id="messagingmainnetworksgrid" style="display:block;border:none;"></div>
	<br style="clear:both;" />
</div>
<script>
	...
	myTab.layout.cells('b').setHeight(<?php echo getOption('$NETWORKS_MAIN_HEIGHT',300); ?>);
	...
	function messagingmainnetworksgrid_%formval%(f) {
...
///////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=messagingmainnetworksgrid&module=messaging&table=network&formval=%formval%",
			...
			var myGrid = myGrid_%formval% = new dhtmlXGridObject('messagingmainnetworksgrid');

			myGrid.setImagePath("/codebase/imgs/")

			myGrid.setHeader("#master_checkbox,ID, Number, Network");

			myGrid.attachHeader("&nbsp;,&nbsp;,#text_filter,#combo_filter");

			myGrid.setInitWidths("50,70,250,*");
			...
			try {
				...
				if(ddata.rows[0].id) {
					...
					myGrid.attachEvent("onRowSelect",function(rowId,cellIndex){
						...
						myTab.postData('/'+settings.router_id+'/json/', {
							odata: {},
							pdata: "routerid="+settings.router_id+"&action=formonly&formid=messagingdetailsnetworks&module=messaging&method=onrowselect&rowid="+rowId+"&formval=%formval%",
						}, function(ddata,odata){
							$ = jQuery;
				...
				$("#formdiv_%formval% #messagingmainnetworksgrid div.objbox").html('Data Not Available');

				myTab.postData('/'+settings.router_id+'/json/', {
					odata: {},
					pdata: "routerid="+settings.router_id+"&action=formonly&formid=messagingdetailsnetworks&module=messaging&method=nodata&formval=%formval%",
				}, function(ddata,odata){
					$ = jQuery;
					$("#formdiv_%formval% #messagingdetails").parent().html(ddata.html);
					layout_resize_%formval%();
