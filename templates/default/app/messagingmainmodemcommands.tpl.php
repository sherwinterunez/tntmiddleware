<div id="messagingmain">
	<div id="messagingmainmodemcommandsgrid" style="display:block;border:none;"></div>
	<br style="clear:both;" />
</div>
<script>
	...
	myTab.layout.cells('b').setHeight(<?php echo getOption('$MODEMCOMMANDS_MAIN_HEIGHT',300); ?>);
	...
	function messagingmainmodemcommandsgrid_%formval%(f) {
...
///////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=messagingmainmodemcommandsgrid&module=messaging&table=modemcommands&formval=%formval%",
			...
			var myGrid = myGrid_%formval% = new dhtmlXGridObject('messagingmainmodemcommandsgrid');

			myGrid.setImagePath("/codebase/imgs/")

			myGrid.setHeader("#master_checkbox,ID, Name, Description");

			myGrid.setInitWidths("50,70,200,*");
			...
			try {
				...
				if(ddata.rows[0].id) {
					...
					myGrid.attachEvent("onRowSelect",function(rowId,cellIndex){
						...
						myTab.postData('/'+settings.router_id+'/json/', {
							odata: {},
							pdata: "routerid="+settings.router_id+"&action=formonly&formid=messagingdetailsmodemcommands&module=messaging&method=onrowselect&rowid="+rowId+"&formval=%formval%",
						}, function(ddata,odata){
							$ = jQuery;
				...
				$("#formdiv_%formval% #messagingmainmodemcommandsgrid div.objbox").html('Data Not Available');

				myTab.postData('/'+settings.router_id+'/json/', {
					odata: {},
					pdata: "routerid="+settings.router_id+"&action=formonly&formid=messagingdetailsmodemcommands&module=messaging&method=nodata&formval=%formval%",
				}, function(ddata,odata){
					$ = jQuery;
					$("#formdiv_%formval% #messagingdetails").parent().html(ddata.html);
					layout_resize_%formval%();
