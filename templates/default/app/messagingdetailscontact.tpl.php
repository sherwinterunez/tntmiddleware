...
$toolbars = array('messagingcomposeto','messagingrefresh','messagingimport','messagingexport');

if(in_array('contactsnew',$access)) {
	$toolbars[] = 'messagingnew';
	$savecancel = true;
}

if(in_array('contactsedit',$access)) {
	$toolbars[] = 'messagingedit';
	$savecancel = true;
}

if(in_array('contactsdelete',$access)) {
...
<div id="messagingdetails">
	<div id="messagingdetailscontact" class="navbar-default-bg">
		...
		<div id="messagingdetailscontactdetailsformupload_%formval%" style="display:none;">
		<iframe frameborder="0" border="0" style="position: absolute; width:0px; height:0px; top:-10px; left:-10px;" name="dhxform_%formval%"></iframe>
		<form class="dhxform_upload_form" target="dhxform_%formval%" enctype="multipart/form-data" method="POST" action="/app/json/">
		<input type="hidden" name="routerid" value="<?php echo $vars['post']['routerid']; ?>">
		<input type="hidden" name="action" value="<?php echo $vars['post']['action']; ?>">
		<input type="hidden" name="formid" value="<?php echo $vars['post']['formid']; ?>">
		<input type="hidden" name="module" value="<?php echo $vars['post']['module']; ?>">
		<input type="hidden" name="method" value="messagingupload">
		<input type="hidden" name="formval" value="%formval%">
		<input class="dhxform_image_input" type="file" name="file">
		</form>
		</div>
		<br style="clear:both;" />
	</div>
...
<script>
	...
	function messagingdetailscontact_%formval%() {
		...
		<?php /* ?>
		//var myToolbar = ['messagingcomposeto','messagingsave','messagingcancel','messagingrefresh','messagingnew','messagingedit','messagingdelete'];
		<?php */ ?>
...
///////////////////////////////////
		...
		myTab.toolbar.getToolbarData('messagingcomposeto').onClick = function(id,formval) {
			...
			myTab.postData('/'+settings.router_id+'/json/', {
				odata: {},
				pdata: "routerid="+settings.router_id+"&action=formonly&formid=messagingmaincompose&module=messaging&from=contacts&rowid="+rowid+"&rowids="+rowids+"&formval=%formval%",
		...
		jQuery("#messagingdetailscontactdetailsformupload_%formval% iframe").load(function(){
			//alert('hello, sherwin!');
			...
			if(content.length>0) {
				...
				if(obj.return_message) {
					messagingmaincontactsgrid_%formval%();
					showAlert(obj.return_message);
				}
			}
		});

		jQuery("#formdiv_%formval% #messagingdetailscontactdetailsformupload_%formval% .dhxform_image_input").change(function(){
			//alert(jQuery(this).val());
			$(this).closest("form").submit();
		});

		myTab.toolbar.getToolbarData('messagingimport').onClick = function(id,formval) {
			jQuery("#formdiv_%formval% #messagingdetailscontactdetailsformupload_%formval% .dhxform_image_input").click();
		};
		...
		myTab.toolbar.getToolbarData('messagingexport').onClick = function(id,formval) {
			...
			myTab.postData('/'+settings.router_id+'/json/', {
				odata: {},
				pdata: "routerid="+settings.router_id+"&action=formonly&formid=messagingdetailscontact&module=messaging&method="+id+"&rowid="+rowid+"&rowids="+rowids+"&formval=%formval%",
			}, function(ddata,odata){
