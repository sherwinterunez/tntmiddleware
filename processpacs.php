...
if(!empty($result['rows'][0]['pacsimage_id'])) {
  ...
  foreach($result['rows'] as $k=>$v) {
	...
	if(count($ret)>=$limit) {
		...
		while(1) {
			$ret[] = array('id'=>0,'conn'=>0);
			if(count($ret)>=$limit) break;
		}
	}
...
if(!($result = $appdb->query("update tbl_pacsimage set pacsimage_status=0 where pacsimage_id in (select pacsimage_id from (select *,(extract(epoch from now()) - extract(epoch from pacsimage_updatestamp)) as elapsedtime from tbl_pacsimage where pacsimage_status=20 order by pacsimage_id asc) as a where elapsedtime>3600)"))) {
  //echo "\n0 message. processOutbox done.\n";
  return false;
}
