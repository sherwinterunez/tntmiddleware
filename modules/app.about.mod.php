...
if(!class_exists('APP_app_about')) {

	class APP_app_about extends APP_Base_Ajax {
		...
		function add_rules() {
			...
			$appaccess->rules($this->desc,'About Module');
			$appaccess->rules($this->desc,'About Module New');
			$appaccess->rules($this->desc,'About Module Edit');
			$appaccess->rules($this->desc,'About Module Delete');
		...
		function _form_about($routerid=false,$formid=false) {
			...
			if(!empty($routerid)&&!empty($formid)) {
				...
				if(!empty($post['method'])&&($post['method']=='aboutedit')) {
					$readonly = false;
				}

				if(!empty($post['method'])&&$post['method']=='aboutsave') {
					$retval = array();
					$retval['return_code'] = 'SUCCESS';
					$retval['return_message'] = 'About successfully saved!';
					$retval['post'] = $post;
