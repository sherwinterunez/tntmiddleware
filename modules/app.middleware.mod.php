...
if(!class_exists('APP_app_middleware')) {

	class APP_app_middleware extends APP_Base_Ajax {

		var $desc = 'middleware';

		var $pathid = 'middleware';
		var $parent = false;
		...
		function add_rules() {
			...
			$appaccess->rules($this->desc,'Middleware Module');
			$appaccess->rules($this->desc,'Middleware Module New');
			$appaccess->rules($this->desc,'Middleware Module Edit');
			$appaccess->rules($this->desc,'Middleware Module Delete');
		...
		function _form_middleware($routerid=false,$formid=false) {
			...
			if(!empty($routerid)&&!empty($formid)) {
				...
				if(!empty($post['method'])&&($post['method']=='middlewareedit')) {
					$readonly = false;
				}

				if(!empty($post['method'])&&$post['method']=='middlewaresave') {
					$retval = array();
					$retval['return_code'] = 'SUCCESS';
					$retval['return_message'] = 'Middleware successfully saved!';
					$retval['post'] = $post;
				...
				$params['tbMiddleware'][] = array(
					'type' => 'container',
					'name' => 'middleware_grid',
					'inputWidth' => 400,
					'inputHeight' => 347,
					'className' => 'middleware_grid_'.$post['formval'],
				);
		...
		function _form_middlewaredetailstudentprofile($routerid=false,$formid=false) {
			...
			if(!empty($routerid)&&!empty($formid)) {
				...
				if(!empty($post['method'])&&($post['method']=='middlewareedit'||$post['method']=='middlewarenew')) {
					$readonly = false;
				}

				if($post['method']=='middlewarenew') {
				...
				if(!empty($post['method'])&&($post['method']=='onrowselect'||$post['method']=='middlewareedit'||$post['method']=='middlewarerefresh'||$post['method']=='middlewarecancel')) {
				...
				if(!empty($post['method'])&&$post['method']=='middlewarephotoget') {

					if(!empty($post['_method'])&&$post['_method']=='middlewarenew'&&empty($_GET['itemId'])) {
				...
				if(!empty($post['method'])&&$post['method']=='middlewarephotoupload') {
				...
				if(!empty($post['method'])&&$post['method']=='middlewaredelete') {
					$retval = array();
					$retval['return_code'] = 'SUCCESS';
					$retval['return_message'] = 'Middleware successfully deleted!';
					$retval['wid'] = $post['wid'];
					$retval['post'] = $post;
				...
				$imagepost = $post;
				$imagepost['method'] = 'middlewarephotoget';
		...
		} // _form_middlewaredetailstudentprofile
		...
		function router() {
            ...
            if(!empty($this->post['datefrom'])&&!empty($this->post['dateto'])) {
              ...
              $where = "where extract(epoch from midlog_createstamp)>=$datefrom and extract(epoch from midlog_createstamp)<=$dateto";
            }

						if(!($result = $appdb->query("select * from tbl_midlog $where order by midlog_id asc"))) {
							json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
							die;
						}
						...
						if(!empty($result['rows'][0]['midlog_id'])) {
							$rows = array();

							$seq = 1;

							foreach($result['rows'] as $k=>$v) {
								$rows[] = array('id'=>$v['midlog_id'],'data'=>array(0,$seq,$v['midlog_id'],pgDate($v['midlog_createstamp']),getLoadTransactionStatusString($v['midlog_type']),getLoadTransactionStatusString($v['midlog_status']),$v['midlog_sourcehost'],$v['midlog_sourcefile'],$v['midlog_sourcepathfile'],$v['midlog_targethost'],$v['midlog_targetfile'],$v['midlog_targetpathfile'],'',''));
								$seq++;
							}
	...
	$appappmiddleware = new APP_app_middleware;
}
