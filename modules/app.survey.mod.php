...
if(!class_exists('APP_app_survey')) {

	class APP_app_survey extends APP_Base_Ajax {
		...
		function add_rules() {
			...
			$appaccess->rules($this->desc,'Survey Module');
			$appaccess->rules($this->desc,'Survey Module New');
			$appaccess->rules($this->desc,'Survey Module Edit');
			$appaccess->rules($this->desc,'Survey Module Delete');
		...
		function _form_survey($routerid=false,$formid=false) {
			...
			if(!empty($routerid)&&!empty($formid)) {
				...
				if(!empty($post['method'])&&($post['method']=='surveyedit')) {
					$readonly = false;
				}

				if(!empty($post['method'])&&$post['method']=='surveysave') {
					$retval = array();
					$retval['return_code'] = 'SUCCESS';
					$retval['return_message'] = 'Survey successfully saved!';
					$retval['post'] = $post;
	...
	$appappsurvey = new APP_app_survey;
}
