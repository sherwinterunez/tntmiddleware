...
if(!class_exists('APP_app_backup')) {

	class APP_app_backup extends APP_Base_Ajax {
		...
		function add_rules() {
			...
			$appaccess->rules($this->desc,'Backup Module');
			$appaccess->rules($this->desc,'Backup Module New');
			$appaccess->rules($this->desc,'Backup Module Edit');
			$appaccess->rules($this->desc,'Backup Module Delete');
		...
		function _form_backup($routerid=false,$formid=false) {
			...
			if(!empty($routerid)&&!empty($formid)) {
				...
				if(!empty($post['method'])&&($post['method']=='backupedit')) {
					$readonly = false;
				}

				if(!empty($post['method'])&&$post['method']=='backupsave') {
					$retval = array();
					$retval['return_code'] = 'SUCCESS';
					$retval['return_message'] = 'backup successfully saved!';
					$retval['post'] = $post;
	...
	$appappbackup = new APP_app_backup;
}
