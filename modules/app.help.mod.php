...
if(!class_exists('APP_app_help')) {

	class APP_app_help extends APP_Base_Ajax {
		...
		function add_rules() {
			...
			$appaccess->rules($this->desc,'Help Module');
			$appaccess->rules($this->desc,'Help Module New');
			$appaccess->rules($this->desc,'Help Module Edit');
			$appaccess->rules($this->desc,'Help Module Delete');
		...
		function _form_help($routerid=false,$formid=false) {
			...
			if(!empty($routerid)&&!empty($formid)) {
				...
				if(!empty($post['method'])&&($post['method']=='helpedit')) {
					$readonly = false;
				}

				if(!empty($post['method'])&&$post['method']=='helpsave') {
					$retval = array();
					$retval['return_code'] = 'SUCCESS';
					$retval['return_message'] = 'Help successfully saved!';
					$retval['post'] = $post;
