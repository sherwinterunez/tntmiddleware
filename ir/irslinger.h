...
static inline void addPulseB(uint32_t onPins, uint32_t offPins, uint32_t duration, gpioPulse_t *irSignal, int pulseCount)
{
        int index = pulseCount;
...
static inline int myIR2(uint32_t outPin, int frequency, double dutyCycle) {
	...
	gpioPulse_t irSignal[MAX_PULSES];
        ...
        int totalCycles = (int)round(frequency); //(int)round(duration / oneCycleTime);
        int totalPulses = 2; //totalCycles * 2;
        ...
        for (i = 0; i < totalPulses; i++)
        {
                ...
                {
                        // High pulse
                        addPulseB(1 << outPin, 0, onDuration, irSignal, pulseCount);
                }
                else
                {
                        // Low pulse
                        addPulseB(0, 1 << outPin, offDuration, irSignal, pulseCount);
                }

                pulseCount++;
        }

        printf("totalCycles: %i\n", totalCycles);
	...
	gpioWaveAddGeneric(2, irSignal);

        int waveID = gpioWaveCreate();

        if (waveID >= 0)
        {
                int result = gpioWaveTxSend(waveID, PI_WAVE_MODE_REPEAT);
...
static inline int myIR(uint32_t outPin, int frequency, double dutyCycle) {
